function [window] = window_generator(input)
    ...
    window(i, 1:input.sample_numbers + input.zeros_symmetric + 1) = [component, zeros(1,input.zeros_symmetric)];
    end
    ...
    if (input.window_plot_status)
        ...
        for i = 1:window_components
            window_magnitude_fft(i, 1:input.sample_numbers + input.zeros_symmetric) = abs(fft(window(i, 1:input.sample_numbers + input.zeros_symmetric))/(input.sample_numbers + input.zeros_symmetric));
        ...
        figure;
        n = 0:input.sample_numbers + input.zeros_symmetric - 1;
